...
class VocoderController : public Controller {
    ...
    private:
        LinearInput<> resonanceInput = LinearInput<>(AnalogPin(A0), -5, 5, 0, 1);
        ExpInput<> centreFrequencyInput = ExpInput<>(AnalogPin(A1));
        LinearInput<> pitchIntervalInput = LinearInput<>(AnalogPin(A2), -5, 5, 0.1, 1.0);
        LinearInput<> stereoMixInput = LinearInput<>(AnalogPin(A3), -5, 5, 0, 1);
